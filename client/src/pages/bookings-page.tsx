    ...
    <div className="flex flex-col items-center min-h-screen p-4">
      <div className="w-full max-w-6xl">
        ...
        <Tabs defaultValue="all" onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-3 w-full max-w-md mb-6 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-lg shadow-sm">
            <TabsTrigger 
              value="all" 
            ...
            {isLoading ? (
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {filteredBookings.map((booking) => {
                  ...
                  return (
                    <Card key={booking.id} className="overflow-hidden border-t-4 hover:shadow-md transition-shadow flex flex-col" style={{ borderTopColor: 'var(--primary)' }}>
                      <CardHeader className="pb-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <CardTitle className="text-lg font-bold">{bookingInfo.title}</CardTitle>
                            <CardDescription className="flex items-center mt-1">
                              <MapPin className="h-4 w-4 mr-1 flex-shrink-0" />
                              <span className="truncate">{bookingInfo.location}</span>
                            </CardDescription>
                          </div>
                          <Badge variant={getStatusBadgeVariant(booking.status)} className="ml-2 flex-shrink-0">
                      ...
                      <CardContent className="pb-4 flex-grow">
                        <div className="space-y-3">
                          ...
                          {booking.bookingDuration ? (
                            ...
                            <div className="h-5"></div> /* Spacer for consistent height */
                          )}
                      ...
                      <CardFooter className="bg-slate-50 dark:bg-slate-900 flex justify-between py-3 border-t mt-auto">
                        <div className="text-xs text-muted-foreground">
                          Booking #{booking.id} • {bookingInfo.type}
                        </div>
